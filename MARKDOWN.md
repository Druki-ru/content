...
- `path`: (опционально) Позволяет переопределить алиас для материала. Пригодится если делается материал из одного цикла и нужно соблюдать его структуру. Данный путь указывается полность, автоматический шаблон `/docs/[core:version]/[title]` полностью игнорируется и вам нужно указывать путь полностью.
...
_К моменту написания этого файла, идея оказалась крайне сомнительной, но пока жива и используется, с огромной вероятностью это поведение будет изменено полностью, в пользу штатных ссылок. Советы и обсуждения принимаются в ишьюсах._
