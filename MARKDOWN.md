...
CommonMark ничем не выделяется от стандартного Markdown, кроме того, что он задокументирован и стандартизован. Если вы знакомы с Markdown, то вас заинтересует только [раздел собственных расширений и синтаксиса](#custom-syntax). Тем не менее, рекомендуется пробежаться по основному синтаксису, чтобы узнать о дополнительных требованиях в пределах проекта.
...
Не допускается использование заголовка 1 уровня, так как он автоматически создается системой на странице из мета-информации. Внутри материала используйте какие хотите, но, желательно, в пределах 2-4 уровней.
...
Обычные ссылки составляют из двух частей:
...
Конечные ссылки будут в дальнейшем заменены на настоящие URL материалов и их алиасы, так что, всё что нужно, сослаться на нужный файл в репозитории, относительно текущего.
...
```markdown
Вот так пишется "Привет Мир!" в PHP: `echo "Hello World";`.
```

### Многострочный код

Код содержащий несколько строк, должен оборачиваться при помощи утроенного символа `\`` с последующим названием языка программирования, чей код представлен в данном блоке. 
...
Мета-информация описывается в YAML-синтаксисе и полностью его поддерживает. Данный синтаксис должен быть обернут тройным дефисом `-` как в начале мета-информации, так и в конце. Все что находится между ними - должно быть в YAML формате.
...
- `category`: (массив, опционально) Позволяет категоризировать материал по названию категории в пределах `core` версии (без неё тоже будет работать). Принимает следующие значения:
  ...
  - `title`: (опционально) Переопределяет заголовок в меню категории на указанный здесь. Если не указано, будет использоваться значение `title`.
- `path`: (опционально) Позволяет переопределить алиас для материала. Пригодится если делается материал из одного цикла и нужно соблюдать его структуру. Данный путь указывается полностью, автоматический шаблон `/[core:version]/[id]` полностью игнорируется и вам нужно указывать путь полностью. **Обратите внимание:** Данный путь задается без префикса сайта /wiki. Например, для страницы https://druki.ru/wiki/admin/appearance нужно указывать `/admin/appearance`, так как `/wiki` - это особенность сайта, а не содержимого. Указывайте путь с мыслью что вы работаете не с сайтом, а только в пределах данного репозитория с контентом.
...
- `metatags`: (массив, опционально) Данный параметр позволяет задать конкретные мета-теги для материала в принудительном порядке. На данный момент поддерживаются следующие переопределяемые значения:
