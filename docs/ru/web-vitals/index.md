...
В данном разделе собраны рекомендации, применяя которые, вы сможете достичь положительных результатов в проверках.
...
Старайтесь держать Time To First Byte (TTFB) ниже 200мс — [рекомендованное значение](https://developers.google.com/web/tools/chrome-devtools/network/understanding-resource-timing). Значения выше 600мс считаются проверками и сильно влияют и искажают все прочие проверки.

* Используйте встроенное в Drupal кэширование: Dynamic Page Cache, Internal Page Cache. Для большинства небольших сайтов будет достаточно включить Internal Page Cache для достижения рекомендуемых значений.
* Если нет возможности использовать Internal Page Cache, сократите количество кода выполняемого в рантайме и оптимизируйте его, храните значения в cache bin.
* Если данные персональные и не кэшируемые — загружайте их лениво. Вы можете использовать «ленивые строители» и «стратегии рендера плейсхолдеров» для различных подходов. Попробуйте использовать поставляемый в комплекте модуль BigPipe.
* Если инвалидация кэша происходит часто или в больших объёмах, используйте инструменты «разогрева» кэша для самых важных страниц. Например, модуль [Warmer](https://www.drupal.org/project/warmer) позволяет производить разогрев с использованием [очередей](../drupal/9/queues/index.md).
