...
В общем смысле, DI - это контейнер, который подготоваливает объекты запрошенных сервисов, и передает их экземпляры в конструктор объекта, который использует DI.
...
Данный подход не является DI, но это то, как иногда приходится обращаться к сервисам и внедрять их в legacy код. Поэтому, о нём здесь также упомянуто.

В Drupal существует одноименный глобальный объект `\Drupal`, который имеет свой собственный Service Container из которого можно вызывать сервисы. Данный объекто только для этого и предзназначен. Основным методом данного объекта является `::service()`, где в качестве аргумента принимаетяс название сервиса, а в качестве результата, возвращается его экземпляр объекта.

```php
$service = \Drupal::service('example.simple');
```

Данный объект также содержит методы, которые являются алиасами для вызова конкретных сервисов из ядра, которые чаще всего используются. Ознакомиться с ними вы можете внутри объекта.

**Обратите внимание!** Данный способ обращения к сервисам является нежелательным и во многих моментах совершенно неправильным. Подобная возможность предусмотрена только для legacy кода (функций, хуков и т.д., где нет поддержки нормального DI), который ещё не переписан должным образом, чтобы была правильная поддержка DI. Там, где есть возможность использовать один из других способов DI, должны быть использован именно он, а не данный подход.

Злоупотребление данным подходом, также негативно сказывается на производительности.

> [!IMPORTANT]
> Не используйте данный подход там, где есть возможность использовать другие. Это худший из вариантов. Если в вашем коде используется данный подход, рассмотрите вариант рефакторинга его на другой подход, если это возможно.
...
Все плагины, в конечном итоге, наследуются от других объектов. Вы должны учитывать данное поведение при внедрении DI. По умолчанию, все плагины принимают, как миниммум, `$configuration`, `$plugin_id` и `$plugin_definition`. Таким образом, три данных аргумента являются обязательным для плагинов. Данные аргументы принимаются в базовом плагине, поэтому вам также необходим их добавить и передать родительскому объекту.
...
> [!NOTE]
> Обратите внимание, что в конструкторе передаются все необходимые родительскому объекту (`BlockBase`) аргументы. У некоторых блоков родители также могут запрашивать сервисы, и вам необходимо их также передавать. Для этого изучите `__construct()` рдительского объекта плагина.
...
Он также добавляет необходимость описать статический метод `create()`, только, в отличии от DI плагинов, он принимает один единственный аргумент `$container`.
...
Деритивы, которым требуется Dependency Injection, должны реаилзовывать интерфейс `Drupal\Core\Plugin\Discovery\ContainerDeriverInterface`.
...
- `$entity_type`: Объект типа сущности, для которогой данный хендлер вызван.
