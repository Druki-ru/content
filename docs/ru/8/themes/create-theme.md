...
- **experimental**: (опционально, добавлено в [Drupal 8.8.0](../releases/release-8.8.0.md)) Помечает тему как "экспериментальная". Для таких тем будет добавлено уведомление, как у модулей, предупреждающих о возможных проблемах.
