...
metatags:
  title: 'Маршрутизация Drupal 8: Структура маршрутов'
...
- **requirements**: (обазятельно) Определяет, какие условия должны быть удовлетворены, для того чтобы получить доступ к маршруту. Должно быть одно или более значений (все они должны пройти успешно).
  ...
  - Любой другой существующий Access Check сервис. Для более подробной информации обратитесь к разделу {контроля доступа маршрутов}(route-access-control:8).
