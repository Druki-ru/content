...
В массиве `$settings` указываются настройки для Drupal который могут корректировать его поведение.
