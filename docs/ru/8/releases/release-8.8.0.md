...
Прежде чем обновляться, убедитесь, что сервер, на котором работает сайт, имеет версию PHP как минимум 7.0.3 (рекомендуется PHP 7.2+). [Drupal 8.7.0](release-8.7.0.md) последний релиз который поддерживает PHP 5.6.
