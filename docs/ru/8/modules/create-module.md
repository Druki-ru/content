...
search-keywords:
  ...
  - где писать код
  - куда вставлять код
category-area: 'Модули'
category-title: 'Создание модуля'
category-order: 2
---
...
- **configure**: Если ваш модуль имеете настройки, вы можете указать название {маршрута}(routing:8) в данном параметре. Для активного модуля с данным значением, в списке модулей будет дополнительная кнопка для перехода в настройки. Например `configure: hello_world.settings`.
...
Создание composer.json файла позволяет корректно обрабатывать модуль при помощи {Composer}(composer). Данный файл является обязательным если вы собираетесь публиковать модуль на drupal.org, а также он имеет зависимости отличные от ядра.
