...
- [Хуки](../8/hooks/hooks.md) или иные функции, использущие данную функцию обратного вызова, должны ссылаться на её название в документации.
...
- Если в списке требуются ключи, используйте двоеточение между ключем и его описанием. API модуль отформатирует ключи при помощи `<strong>`.

Примечания по Drupal стандартам:

- Строка перед списком должна заканчиваться доветочием `:`.
...
- Если доступно несколько возможных типов, разделяйте их вериткальной чертой (`|`).
...
- Когда вы возваращаете основной объект класса (`$this`), используйте `@return $this`.
...
- Для встроенных в PHP типов используйте следующие имена:
  - `array` (НЕ `Array`). Тем не менее, синатксис `[]` приоритетнее, если это допустимо. То есть, если вы знаете что содержит массив и все его элементы данного типа, то указывайте `string[]` или `\My\Class\Name[]` и т.д., вместо использования общего типа `array`.
