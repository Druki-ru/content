...
В ядро добавлена новая [библиотека](../../../libraries/index.md) `drupal.message`, которую необходимо подключать там, где вы хотите использовать данный API.
...
По умолчанию, перед обновлением схемы, автоматически будут созданы резервные копии изменяемых таблиц в БД, а также сохранены после успешного обновления. Вы можете изменить данное поведение в [settings.php](../../../settings-php/index.md) `$settings['entity_update_backup'] = FALSE;`.
...
Вместо них необходимо использовать соответствующие методы [сервиса](../../../services/index.md) `file_system`.
...
- [#2997122](https://www.drupal.org/node/2997122) Теперь вы можете использовать вызываемые [сервисы](../../../services/index.md) в качестве [контроллеров](../../../routing/index.md), при помощи магического метода [__invoke](https://php.net/manual/en/language.oop5.magic.php#object.invoke).
...
- [#2997808](https://www.drupal.org/node/2997808) Добавлены три новых [хука](../../../hooks/index.md) `hook_aggregator_fetcher_info_alter()`, `hook_aggregator_parser_info_alter()`, `hook_aggregator_processor_info_alter()` для редактирования плагинов модуля Aggregator.
...
- [#3000819](https://www.drupal.org/node/3000819) Атрибут `data-drupal-link-system-path` теперь устанавливается только для системных путей и [маршрута](../../../routing/index.md) `<front>`.
...
- [#3006470](https://www.drupal.org/node/3006470) [Плагины](../../../plugins/index.md) `MigrateField` теперь имеют "вес" (`weight`).
...
- [#3015367](https://www.drupal.org/node/3015367) Добавлен новый [хук](../../../hooks/index.md) `hook_entity_preload()`. Данный хук позволяет подменять данные, прежде чем сущности будут загружены, например, ревизию для загрузки.
