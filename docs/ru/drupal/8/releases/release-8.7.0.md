...
- Перед обновлением на 8.7.0, вам необходимо удалить контриб модуль при помощи [Composer](../../../composer/composer.md) `composer remove drupal/jsonapi`, если не используете composer, то папку с модулем.
...
В ядро добавлена новая [библиотека](../libraries/libraries.md) `drupal.message`, которую необходимо подключать там, где вы хотите использовать данный API.
