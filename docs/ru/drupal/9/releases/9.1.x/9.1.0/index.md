...
В классах `ViewEditForm` и `HtmlRenderer` использование `\Drupal::theme()` заменено на [Dependency Injection](../../../services/dependency-injection/index.md), в связи с чем, в конструкторе появился новый аргумент.
...
Класс `LayoutBuilder` теперь принимает `EventDispatcherInterface $event_dispatcher` вместо `LayoutTempstoreRepositoryInterface $layout_tempstore_repository`. Также удалено внедрение `messenger` [сервиса](../../../services/index.md).
...
Для того чтобы пометить [настройку ядро](../../../settings-php/index.md) устаревшей необходимо добавить информацию в массив `Settings::$deprecatedSettings`.
...
[Сервис](../../../services/index.md) `twig` был расширен для поддержки данной возможности в Twig шаблонах.
...
Текущая тема оформления — Bartik, была частью релиза Drupal 7, который состоялся в январе 2011 года. Спустя 9 лет, данная тема успела попасть в релизы [Drupal 8](../../../../8/index.md) и [Drupal 9](../../../index.md). Веб двигается вперёд, это было отлично для 2011 года, но не годится сейчас.
...
Начиная с [Drupal 8.8](../../../../8/releases/8.8.x/8.8.0/index.md) и до [Drupal 9.0](../../9.0.x/9.0.0/index.md), включительно, Composer шаблоны для Drupal ([drupal/recommended-project
...
Для проверки обновлений по HTTP добавлена новая [настройка](../../../settings-php/index.md) `update_fetch_with_http_fallback`. Это позволит работать в прежнем режиме и получать информацию об обновлениях. Включение данной настройки может привести к возможности для MITM атак на сайт.
...
- [#2404105](https://www.drupal.org/project/drupal/issues/2404105) Теперь при установке [дистрибутивов](../../../distributions/index.md) блоки устанавливается только для конкретной темы, а не для всех активных.
...
- [#2278971](https://www.drupal.org/node/2278971) `Connection::supportsTransactions` помечен устаревшим. Таким образом [настройка](../../../settings-php/index.md) подключения к БД `transactions` также становится устаревшей.
...
- [#3087879](https://www.drupal.org/project/drupal/issues/3087879) Для поиска по Help Topic теперь используется административная [тема оформления](../../../themes/index.md).
...
- [#3069578](https://www.drupal.org/project/drupal/issues/3069578) Исправлена неполадка из-за которой [псевдо-поля](../../../hooks/extra-fields/index.md) не рендерились.
...
- [#3075703](https://www.drupal.org/project/drupal/issues/3075703) Функции для обработки поискового запроса `search_index_split()`, `search_simplify()` и `search_expand_cjk()` перенесены в [сервис](../../../services/index.md) `search.text_processor`.
