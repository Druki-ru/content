...
Пример реализации хука `hook_install()`, который вызывается в процессе установки [модуля](../modules/modules.md) в котором объявлен:
...
Пример реализации хука `hook_form_alter()`, который вызывается при подготовке любой формы с использованием [Form API](../forms/forms.md) и позволяет изменять форму прежде чем она будет отрендерена:
...
У хуков нет централизованного хранилища или менеджера поэтому их вызов и обработка может находиться абсолютно где угодно.
