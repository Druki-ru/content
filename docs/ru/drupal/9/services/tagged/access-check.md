...
- **Node:**
  - `access_check.node.revision` (`_access_node_revision`): Производит проверку, может ли пользоваеть просматривать ревизии содержимого.
  - `access_check.node.add` (`_node_add_access`): Производит проверку, может ли пользоваеть просматривать страницы создания содержимого.
  - `access_check.node.preview` (`_node_preview_access`): Производит проверку, может ли пользоваеть использовать предпросмотр содержимого.
- **Quickedit:**
...
- **User:**
  ...
  - `access_check.user.login_status` (`_user_is_logged_in`): Производит проверку, является ли пользователь авторизованым.
- **Workflows:**
...
## Использование на маршуртах

Для того чтобы использовать Access Check на маршрутах, их необходимо указать в параметре `requirements` с любым значением. По умолчани, если Access Check не использует своё значение как аргумент, принято указывать `'TRUE'`.
...
Если вы хотите дать возможность передавать примитивные значения при указании требования в маршурете, вы можете получать их из `$route` объекта.
