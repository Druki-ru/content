...
slug: wiki/10/themes/create
core: 10
metatags:
  title: 'Drupal 10: Создание темы оформления'
...
- **starterkit**: (по умолчанию `false`) Если задать `true`, тема будет являться «стартовой» и использована как каркас для создания новой темы. Темы помеченные как `starterkit` становятся доступны при использовании команды `drupal generate-theme --starterkit STARTER_THEME_NAME NEW_THEME_NAME`.
