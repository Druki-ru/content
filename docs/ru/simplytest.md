...
Для работы с сервисом вам потребуется активное подключение к интернету. Данный сервис часто используется чтобы посмотреть на {будущие версии Drupal}(changelog:8) или протестировать работу какого-то из модулей.
...
> [!NOTE]
> Сервис совершенно бесплатный и не гарантирует стабильность в работе. Временами сервис находится под большой нагрузкой и возможны проблемы в работе.
...
1. Поле ввода {модуля}(modules:8), {темы}(themes:8) или {дистрибутива}(distributions:8) который вы желаете установить. Если вы желаете установить только ядро, или ядро с несколькими модулями, выберите "Drupal Core".
...
1. Элемент для добавления ссылок на потчи в формате `https://example.com/path/to/patch.patch`. Данные патчи будут применены при загрузке соответствующих зависимостей.
...
1. В поле "Enter a project name" укажите "Drupal Core" и выберите подходящую верси. Рекомендуется использовать {актуальную версию}(changelog:8).
...
1. Нажмите кнопку "Launch sandbox" и дождитесь окончания процесса. Это может занять несколько минут.
  - Если вы нажали "Bypass Installation" у вас запуститься ручной процесс установки Drupal. Рекомендуем обратиться к {руководству по установке Drupal}(installation:8).
